import { OutboxDatabase } from './database';
import type {
  MarkAckedInput,
  MarkSendingInput,
...
import { SETTINGS_SNAPSHOT_DEFAULT_KEY } from './types';

type SqlValue = string | number | null;

export async function openOutboxStorage(options: OpenOutboxStorageOptions): Promise<OutboxStorageHandle> {
  const db = await OutboxDatabase.open(options);
  return new SqlOutboxRepository(db);
}

export async function getOutboxStorageMigrationStatus(options: OpenOutboxStorageOptions): Promise<StorageMigrationStatus> {
  const db = await OutboxDatabase.open(options);
  try {
...
class SqlOutboxRepository implements OutboxStorageHandle {
  ...
  async listEventIDsByState(state: OutboxEventRow['deliveryState'], limit: number): Promise<string[]> {
    if (!Number.isFinite(limit) || limit <= 0) {
      return [];
    }
    const rows = this.db.query(
  ...
  async markSending(input: MarkSendingInput): Promise<number> {
  ...
  async markAcked(input: MarkAckedInput): Promise<number> {
    if (input.eventIds.length === 0) {
      return 0;
    }
    return this.db.withTransaction(() => {
  ...
  async revertSendingToPending(input: RevertSendingInput): Promise<number> {
    if (input.eventIds.length === 0) {
      return 0;
    }
    return this.db.withTransaction(() => {
  ...
  async moveToQuarantine(input: QuarantineInput[]): Promise<number> {
    if (input.length === 0) {
      return 0;
    }
  ...
  async readSettingsSnapshot(snapshotKey = SETTINGS_SNAPSHOT_DEFAULT_KEY): Promise<SettingsSnapshotRow | null> {
    const rows = this.db.query(
      `
  ...
  async getStats(): Promise<OutboxStorageStats> {
...
function mapOutboxEventRow(row: Record<string, string | number | null>): OutboxEventRow {
  return {
...
function toDeliveryState(value: string | number | null): OutboxEventRow['deliveryState'] {
...
function toStringField(value: string | number | null, field: string): string {
...
function toNullableStringField(value: string | number | null): string | null {
  if (value === null) {
    return null;
  }
  if (typeof value === 'string') {
...
function toNumberField(value: string | number | null, field: string): number {
