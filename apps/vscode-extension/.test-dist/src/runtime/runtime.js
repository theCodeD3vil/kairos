...
exports.KairosRuntime = void 0;
const constants_1 = require("./constants");
const filters_1 = require("./filters");
const outbox_limits_1 = require("./outbox-limits");
const mirror_1 = require("./settings/mirror");
...
class KairosRuntime {
    ...
    activeContextKey;
    qualifiedContextKey;
    lastHandshakeResponse;
    outboxLimitStatus = {
        estimatedSizeBytes: 0,
        thresholdState: 'normal',
        captureBlockedByHardCap: false,
        thresholds: outbox_limits_1.DEFAULT_OUTBOX_THRESHOLDS,
    };
    constructor(options) {
        ...
        this.settingsMirror = new mirror_1.ExtensionSettingsMirror({
        ...
        this.syncWorker = new sync_worker_1.OutboxSyncWorker({
    ...
    async updateActiveEditor(context) {
        ...
        if (nextContextKey !== this.activeContextKey) {
            ...
            if (context && (0, filters_1.isCodingContext)(context) && nextContextKey) {
                this.dwellHandle = this.scheduler.setTimeout(() => {
                    if (this.disposed || this.activeContextKey !== nextContextKey || !this.activeContext || !(0, filters_1.isCodingContext)(this.activeContext)) {
                        return;
                    }
    ...
    async setWindowFocused(focused) {
    ...
    async recordOpen(context) {
        await this.recordEvent('open', context);
    }
    async recordSave(context) {
        await this.recordEvent('save', context);
    }
    async recordEdit(context) {
    ...
    async connectAndSync(options) {
        ...
        try {
            ...
            if (response.protocolVersion !== constants_1.DESKTOP_PROTOCOL_VERSION) {
                throw new Error(`desktop protocol mismatch: expected v${constants_1.DESKTOP_PROTOCOL_VERSION}, received v${response.protocolVersion}`);
            }
    ...
    async recordEvent(eventType, context) {
        if (this.disposed) {
            return;
        }
        this.activeContext = context;
        if (!(0, filters_1.isCodingContext)(context)) {
            this.observer.logInfo(`Skipped ${eventType} event: non-coding context`);
            return;
        }
        const settings = this.effectiveSettings;
        const decision = (0, filters_1.shouldEmitEvent)(eventType, settings, this.focused);
        if (!decision.allowed) {
            this.observer.logInfo(`Skipped ${eventType} event: ${decision.reason}`);
            return;
        }
        if (settings.respectDesktopExclusions && (0, filters_1.isExcludedContext)(context, this.environment.machine, settings.exclusions)) {
        ...
        const shapedContext = (0, filters_1.shapeContextForEmission)(context, settings);
        const event = (0, filters_1.createActivityEvent)(eventType, shapedContext, (0, filters_1.sanitizeMachine)(this.environment.machine, settings.sendMachineAttribution), this.environment.now(), this.environment.randomID());
        ...
        if (enqueueOutcome.kind === 'blocked_hard_cap') {
            this.observer.logWarn(`Skipped ${eventType} event: outbox hard cap reached (${formatBytes(this.outboxLimitStatus.thresholds.hardCapBytes)})`);
            this.publishStatus();
    ...
    resetConnectionProbeSchedule() {
        ...
        }, constants_1.CONNECTION_PROBE_INTERVAL_MS);
    }
    async runConnectionProbe() {
    ...
    scheduleRetryIfNeeded() {
        ...
        this.retryDelayMs = Math.min(this.retryDelayMs * 2, constants_1.MAX_RETRY_DELAY_MS);
        ...
        this.setState(this.queueSize > 0 && this.effectiveSettings.bufferEventsWhenOffline ? 'offline-buffering' : 'retrying', `Retrying in ${Math.round(delayMs / 1000)}s`);
    }
    async refreshOutboxHealth() {
    ...
    applyOutboxLimitStatus(nextStatus) {
    ...
    recordTrackedActivity(eventType, at) {
        if (eventType !== 'edit') {
            return;
        }
...
exports.KairosRuntime = KairosRuntime;
