...
function PlatformButton({ os, ext, href }: { os: string; ext: string; href: string }) {
  return (
    ...
    >
      <span className="inline-flex items-center justify-center gap-3 bg-ink text-paper border-2 border-ink rounded-brutal shadow-brutal px-8 py-5 font-display font-bold text-xl uppercase tracking-wider transition-[transform,box-shadow,background-color,color] [transition-duration:120ms] ease-slam hover:translate-x-[4px] hover:translate-y-[4px] hover:shadow-brutal-sm hover:bg-signal hover:text-ink">
        {os}
...
export function DownloadCTA() {
  return (
    <section
      id="download"
      className="relative bg-paper text-ink py-32 md:py-44 px-6 md:px-10 grid-overlay overflow-hidden"
    >
      <div className="mx-auto max-w-5xl text-center space-y-10">
        <h2 className="font-display font-extrabold text-[clamp(56px,11vw,160px)] leading-[0.9] tracking-tight">
          Try Kairos today.
        </h2>
        <p className="font-display font-bold text-2xl md:text-3xl text-ink/70">
          Free. Open. Yours.
        </p>
        ...
        <p className="font-sans text-sm text-ink/50 pt-8">
