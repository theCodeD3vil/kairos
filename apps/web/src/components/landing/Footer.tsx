...
export function Footer() {
  return (
    <footer className="relative bg-ink text-paper pt-16 pb-8 px-6 md:px-10">
        ...
        <div>
          <div className="inline-flex items-center gap-3">
            <img
              ...
              className="h-9 w-9"
            />
            <p className="font-display font-extrabold text-3xl md:text-4xl tracking-tight">KAIROS</p>
          </div>
          <p className="mt-2 font-sans text-paper/70 text-sm max-w-xs">
            An open source alternative for tracking coding time locally.
          </p>
        </div>
        <ul className="flex flex-wrap gap-x-8 gap-y-3 font-display text-sm font-bold">
          {FOOTER_LINKS.map((l) => (
            <li key={l.label}>
              <a
                href={l.href}
      ...
      <div className="mx-auto max-w-7xl mt-12 pt-4 border-t border-signal/80 font-sans text-xs text-paper/50">
