    ...
    (window as unknown as { go?: Record<string, unknown> }).go = {
      main: {
        App: {
          ...
          ExportLocalDataToDisk: () => Promise.resolve({
            filePath: '/tmp/kairos-export.json',
            cancelled: false,
            machineCount: 1,
            eventCount: 0,
            sessionCount: 0,
          }),
          PreviewImportLocalDataFromDisk: () => Promise.resolve({ cancelled: true }),
          ImportLocalDataFromDisk: () => Promise.resolve({ cancelled: true }),
